...
with open('padded_data/data_length.pickle'.format(data_number), 'rb') as f:
...
with open('padded_data/data_length.pickle', 'rb') as f:
    data_indice = pickle.load(f)

for data_index in data_indice:
    with open('./padded_data/data_{}.pickle'.format(data_number), 'rb') as f:
        data = pickle.load(f)
    ...
    for j, day in tqdm(enumerate(data), total=len(data)):
        ...
        for i in range(len(day) - (piece_length + looking_forward_length)):
            if i % sampling_frequency == 1:
                continue

            reference_closing_price = day[piece_length + i, 3]
            high_price_forward = day[piece_length + i:piece_length + i + looking_forward_length, 1]
            low_price_forward = day[piece_length + i:piece_length + i + looking_forward_length, 2]
        ...
        if (j % (8000 * sampling_frequency) == 0 and j) or j == len(data) - 1:
            ...
            preprocessed_x = preprocessed_x.tolist()
            preprocessed_y = preprocessed_y.tolist()
            
            np.save('cleaved_data/x_{}_{}.npy'.format(data_number, j), preprocessed_x)
            np.save('cleaved_data/y_{}_{}.npy'.format(data_number, j), preprocessed_y)
